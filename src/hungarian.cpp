...
void hungarian_print_assignment(hungarian_problem_t* p) { hungarian_print_matrix(p->assignment, p->num_rows, p->num_cols); }
...
int hungarian_solve(hungarian_problem_t* p) {
	...
	for (i = 0; i < p->num_rows; ++i)
		for (j = 0; j < p->num_cols; ++j) p->assignment[i][j] = HUNGARIAN_NOT_ASSIGNED;
						...
						if (slack[l]) {
							...
							if (del < slack[l]) {
								if (del == 0) {
									...
									slack[l] = 0;
									parent_row[l] = k;
									if (verbose) fprintf(stderr, "node %d: row %d==col %d--row %d\n", t, row_mate[l], l, k);
			...
			for (l = 0; l < n; l++)
				if (slack[l]) {
					...
					if (slack[l] == 0) {
						...
						k = slack_row[l];
						if (verbose) fprintf(stderr, "Decreasing uncovered elements by %d produces zero at [%d,%d]\n", s, k, l);
						if (row_mate[l] < 0) {
							for (j = l + 1; j < n; j++)
